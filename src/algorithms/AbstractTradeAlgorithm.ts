...
> implements ITradeAlgorithm<InputsType, StateType>
{
  ...
  protected async fixStart(inputs: InputsType, state: StateType): Promise<AlgorithmRun> {
    ...
    const algoRun: AlgorithmRun = await analyzer.persistentStorage.algorithmRunsRepository.runOne(name, inputs, state)
    logger.log({
      type: 'info',
  ...
  protected async fixStop(id: number): Promise<AlgorithmRun> {
    ...
    return await analyzer.persistentStorage.algorithmRunsRepository.stopOne(id)
  }
  ...
  protected async fixContinue(id: number): Promise<AlgorithmRun> {
    ...
    return await analyzer.persistentStorage.algorithmRunsRepository.resumeOne(id)
  }
  ...
  protected async fixFinish(id: number): Promise<AlgorithmRun> {
    ...
    return await analyzer.persistentStorage.algorithmRunsRepository.finishOne(id)
  }
  ...
  protected async fixError(id: number, error: Error): Promise<AlgorithmRun> {
    const { name, analyzer, logger } = this
    await this.stop(id)
    const run = await analyzer.persistentStorage.algorithmRunsRepository.storeError(id, error)
    logger.log({
      type: 'error',
  ...
  protected async saveProgress(id: number, progress: StateType): Promise<AlgorithmRun> {
    ...
    return await analyzer.persistentStorage.algorithmRunsRepository.saveProgress(id, progress)
  }

  @HandleError()
  protected async loadProgress(id: number): Promise<AlgorithmRun> {
    const { name, analyzer, logger } = this
    const algoRun: AlgorithmRun | null = await analyzer.persistentStorage.algorithmRunsRepository.loadProgress(id)
